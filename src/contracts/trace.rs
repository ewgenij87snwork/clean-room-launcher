...
pub fn load_control_map(path: &str) -> Result<Vec<ControlRef>, &'static str> {
    ...
    for (index, line) in text.lines().enumerate() {
        ...
        rows.push(ControlRef {
            id: fields[0].into(),
            plan: plan.into(),
            task: task.into(),
            acceptance: fields[5].into(),
            evidence: fields[6].into(),
            disposition: fields[1].into(),
        });
    }
    Ok(rows)
}
...
pub fn validate_control_map(rows: &[ControlRef]) -> Result<(), &'static str> {
    ...
    for row in rows {
        if row.id.is_empty() || row.acceptance.is_empty() || row.evidence.is_empty() {
            return Err("MISSING_TRACE_FIELD");
        }
        if !ids.insert(&row.id) {
            return Err("DUPLICATE_CONTROL");
        }
        if row.disposition == "ADOPT"
            && row.plan.is_empty()
            && !row.evidence.starts_with("authority:")
        {
            return Err("UNREVIEWED_OWNER");
        }
        if row.evidence.contains("stale") || row.acceptance.contains("CONFLICT") {
