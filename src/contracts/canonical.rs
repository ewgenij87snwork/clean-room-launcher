...
pub fn merge(
    ...
    let left_map = left
        .as_object()
        .ok_or(CanonicalError("MERGE_OBJECT_REQUIRED"))?;
    let right_map = right
        .as_object()
        .ok_or(CanonicalError("MERGE_OBJECT_REQUIRED"))?;
    match operation {
        ...
        MergeOperation::DenyUnion => {
            if left_map.keys().any(|key| right_map.contains_key(key)) {
                Err(CanonicalError("MERGE_COLLISION"))
            } else {
                Ok(right.clone())
            }
        }
            ...
            for (key, value) in right_map {
                if let Some(existing) = result.get_mut(key) {
                    let existing_array = existing
                        .as_array_mut()
                        .ok_or(CanonicalError("MERGE_ARRAY_REQUIRED"))?;
                    let incoming = value
                        .as_array()
                        .ok_or(CanonicalError("MERGE_ARRAY_REQUIRED"))?;
                    for item in incoming {
                        if !existing_array.contains(item) {
        ...
        MergeOperation::OrderedChain => {
            ...
            for (key, value) in right_map {
                if result.contains_key(key) {
                    return Err(CanonicalError("MERGE_COLLISION"));
                }
                result.insert(key.clone(), value.clone());
            }
            Ok(Value::Object(result))
...
fn normalize(value: &Value) -> Result<Value, CanonicalError> {
    match value {
        ...
        Value::Number(number) if number.to_string() == "-0" => {
            Err(CanonicalError("CANONICAL_NEGATIVE_ZERO"))
        }
