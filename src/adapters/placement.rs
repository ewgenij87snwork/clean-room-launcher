...
pub fn place_context(
...
) -> Result<PlacementReceipt, PlacementError> {
    let current =
        verify_current(project).map_err(|_| PlacementError("PLACEMENT_SOURCE_UNVERIFIED"))?;
    if current != *manifest {
    ...
    if verify_current(project).map_err(|_| PlacementError("PLACEMENT_SOURCE_REPLACED"))?
        != *manifest
        || project
            .read(&context_path)
            .map_err(|_| PlacementError("PLACEMENT_CONTEXT_UNAVAILABLE"))?
    ...
    let stored = StoredReceipt {
        ...
        outcome: "STAGED_NOT_APPLIED".into(),
    };
    let bytes = serde_json::to_vec(&stored)
        .map_err(|_| PlacementError("PLACEMENT_RECEIPT_SERIALIZE_FAILED"))?;
...
fn ensure_dir(project: &Dir, path: &str) -> Result<(), PlacementError> {
    match project.symlink_metadata(path) {
        ...
        Err(error) if error.kind() == std::io::ErrorKind::NotFound => project
            .create_dir(path)
            .map_err(|_| PlacementError("PLACEMENT_DIRECTORY_CREATE_FAILED")),
...
fn write_new(project: &Dir, path: &str, bytes: &[u8]) -> Result<(), PlacementError> {
    ...
    let mut file = project
        ...
        .map_err(|_| PlacementError("PLACEMENT_WRITE_REFUSED"))?;
    file.write_all(bytes)
        .map_err(|_| PlacementError("PLACEMENT_WRITE_FAILED"))?;
    file.sync_all()
        .map_err(|_| PlacementError("PLACEMENT_SYNC_FAILED"))
}
...
fn verify_owned(
    ...
    let entries = project
        .read_dir(target)
        .map_err(|_| PlacementError("PLACEMENT_OWNERSHIP_CORRUPT"))?
    ...
    if entries.len() != 2
        || entries.iter().any(|entry| {
            !matches!(
                ...
                Some("context.md" | "placement.json")
            )
        })
    {
    ...
    for path in [&receipt_path, &context_path] {
        let metadata = project
            ...
            .map_err(|_| PlacementError("PLACEMENT_OWNERSHIP_MISSING"))?;
    ...
    if stored.schema_version != "taskseal-placement.v1"
        ...
        || stored.context_digest != context_digest
        || sha256_hex(&context) != *context_digest
    {
        return Err(PlacementError("PLACEMENT_OWNERSHIP_MISMATCH"));
    }
    Ok(PlacementReceipt {
        ...
        target: target.into(),
        outcome: PlacementOutcome::OwnedIdentical,
    })
}
