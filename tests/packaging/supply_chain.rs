...
fn metadata_is_subject_bound_complete_and_secret_free() {
    ...
    let temp = std::env::temp_dir().join(format!("clroom-packaging-supply-chain-{}", std::process::id()));
    let _ = fs::remove_dir_all(&temp);
    fs::create_dir_all(&temp).unwrap();
    let artifact = temp.join("clroom-v0.1.0-aarch64-apple-darwin.tar.gz");
    let output = temp.join("metadata");
    fs::write(&artifact, b"clroom-artifact-v1").unwrap();
    ...
    let generate = run(&[
        ...
        "--builder-id", "local://clroom/packaging", "--output", output.to_str().unwrap(),
    ]);
    assert!(generate.status.success(), "{}", String::from_utf8_lossy(&generate.stderr));
    assert_eq!(String::from_utf8(generate.stdout).unwrap(), "CLROOM_PACKAGING_SUPPLY_CHAIN_GENERATE_PASS qualification=NOT_QUALIFIED\n");
    ...
    assert_eq!(String::from_utf8(accepted.stdout).unwrap(), "CLROOM_PACKAGING_SUPPLY_CHAIN_VERIFY_PASS qualification=NOT_QUALIFIED\n");
    ...
    assert!(run(&["generate", "--artifact", artifact.to_str().unwrap(), "--source-commit", "1111111111111111111111111111111111111111", "--target", "aarch64-apple-darwin", "--builder-id", "local://clroom/packaging", "--output", output.to_str().unwrap()]).status.success());
