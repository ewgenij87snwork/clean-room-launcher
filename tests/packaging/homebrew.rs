...
fn real_mode_prepares_only_a_disposable_local_git_source_and_real_current_evidence() {
    ...
    let output = Command::new("python3").current_dir(root()).env("AWS_SECRET_ACCESS_KEY", "must-not-inherit").args(["packaging/homebrew/lifecycle.py", "--brew-source", source.to_str().unwrap(), "--api-cache-source", api_cache.to_str().unwrap(), "--input-contract", input.to_str().unwrap(), "--real-archive", archive.to_str().unwrap(), "--expected-sha256", &digest, "--expected-source-commit", "1111111111111111111111111111111111111111", "--scenario", "require_loopback_server", "--workspace", workspace.to_str().unwrap(), "--output", result.to_str().unwrap()]).output().unwrap();
    ...
    let refused = Command::new("python3").current_dir(root()).args(["packaging/homebrew/lifecycle.py", "--brew-source", source.to_str().unwrap(), "--api-cache-source", api_cache.to_str().unwrap(), "--input-contract", input.to_str().unwrap(), "--real-archive", archive.to_str().unwrap(), "--expected-sha256", &digest, "--expected-source-commit", "1111111111111111111111111111111111111111", "--scenario", "install_archive_fetch_failed", "--workspace", refused_workspace.to_str().unwrap(), "--output", refused_result.to_str().unwrap()]).output().unwrap();
    assert!(!refused.status.success()); let refused_json = fs::read_to_string(refused_result).unwrap();
    assert!(refused_json.contains("\"failure_class\":\"INSTALL_REFUSED\"") && refused_json.contains("\"diagnostic\":\"install_archive_fetch_refused\"") && refused_json.contains("\"cleanup_complete\":true"), "{}", refused_json);
}
