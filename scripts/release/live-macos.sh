...
config_before=$(required_digest "${TASKSEAL_CONFIG_SHA256:-}" CONFIG_SHA256)
provider_before=$(required_digest "${TASKSEAL_PROVIDER_SHA256:-}" PROVIDER_SHA256)
git_before=$(required_digest "${TASKSEAL_GIT_SHA256:-}" GIT_SHA256)
user_files_before=$(required_digest "${TASKSEAL_USER_FILES_SHA256:-}" USER_FILES_SHA256)
config_after=$(required_digest "${TASKSEAL_CONFIG_SHA256_AFTER:-}" CONFIG_SHA256_AFTER)
provider_after=$(required_digest "${TASKSEAL_PROVIDER_SHA256_AFTER:-}" PROVIDER_SHA256_AFTER)
git_after=$(required_digest "${TASKSEAL_GIT_SHA256_AFTER:-}" GIT_SHA256_AFTER)
user_files_after=$(required_digest "${TASKSEAL_USER_FILES_SHA256_AFTER:-}" USER_FILES_SHA256_AFTER)
...
printf '%s' "$image_id" | grep -Eq '^[A-Za-z0-9._:-]+$' || { echo "P08_LIVE_OS_REFUSED:INVALID_CLEAN_IMAGE_ID" >&2; exit 69; }
