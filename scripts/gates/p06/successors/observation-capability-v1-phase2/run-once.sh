...
protected_inventory() {
  {
    for protected_file in "$auth_source" /Users/ysorokin/.codex/config.toml /Users/ysorokin/.codex/AGENTS.md; do
...
network_status=$?
env -i HOME="$temporary_root/home" PATH=/usr/bin:/bin /usr/bin/sandbox-exec -f "$online_profile" \
  /usr/bin/security find-generic-password -s taskseal-phase2-nonexistent >"$temporary_root/keychain.out" 2>"$temporary_root/keychain.err"
keychain_status=$?
set -e
...
/usr/bin/sandbox-exec -f "$extract_profile" /usr/bin/ruby -rjson -e '
...
if test "$login_status" = 0; then
  cp "$root/fixtures/adapters/codex/context-canaries/native/codex-home/AGENTS.md" "$temporary_root/codex-home/AGENTS.md"
  cp "$root/fixtures/adapters/codex/context-canaries/native/ambient-home/AGENTS.md" "$temporary_root/home/AGENTS.md"
  ...
  env -i HOME="$temporary_root/home" XDG_CONFIG_HOME="$temporary_root/xdg" PATH=/usr/bin:/bin GIT_CONFIG_NOSYSTEM=1 GIT_CONFIG_GLOBAL=/dev/null git init -q "$temporary_root/project"
  (cd "$temporary_root/project/task" && env -i HOME="$temporary_root/home" CODEX_HOME="$temporary_root/codex-home" PATH=/usr/bin:/bin \
    /usr/bin/sandbox-exec -f "$offline_profile" "$command" debug prompt-input 'Return only supplied canary codes.') >"$temporary_root/prompt-input.json" 2>"$temporary_root/prompt-input.stderr"
  ...
  for code in "$l0" "$l2" "$l3"; do test "$(rg -o "$code" "$temporary_root/prompt-input.json" | wc -l | tr -d ' ')" = 1; done
