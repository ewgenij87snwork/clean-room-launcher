...
jq -e '
  keys == ["network","output","projection_state","provider_launch","qualification","result","schema_version","sources","subject","task"] and
  ...
  ([.sources[].path] == ["src/adapters/codex/mod.rs","src/adapters/codex/placement.rs","tests/adapters.rs","tests/adapters/codex/placement.rs"]) and
...
git diff --quiet "$subject" -- src/adapters/codex/mod.rs src/adapters/codex/placement.rs tests/adapters.rs tests/adapters/codex/placement.rs
