...
| Claude Code | unreleased v0.4 whole-plugin selector | `--with=plugin:<provider-native-id>` admits exactly one already-installed whole plugin for one launch | Exact real-provider E2E on Claude Code 2.1.273 / macOS Apple Silicon |
...
whole-plugin selector deliberately activates one provider-native plugin bundle
as an atomic unit; it does not perform component-level surgery.
